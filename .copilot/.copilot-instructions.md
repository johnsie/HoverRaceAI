...
## Project Overview
HoverRace is a 3D racing game developed in C++ using SDL2  for graphics rendering and MFC (Microsoft Foundation Classes) for the Windows UI. The project consists of multiple interdependent DLL libraries and the main Game2.exe application.
...
### Quick Build Command (Game2 only)
```powershell
...
- Always check MSBuild exit code. Look for "Error(s)" in output. Warnings are generally OK.
- Before testing, clear all Game2_*.log files to ensure fresh logs are generated.
- If build fails, check that all dependent projects (Model, MainCharacter, etc.) compiled successfully first.
- **CRITICAL: Always verify Game2.exe is newly built before testing.** Check the timestamp of `C:\originalhr2\HoverRaceAI\Release\Game2.exe` to ensure it was rebuilt in the current session. If it's old, use `/t:Rebuild` flag and delete the old exe first: `Remove-Item "C:\originalhr2\HoverRaceAI\Release\Game2.exe" -Force` before rebuilding.
...
- Make sure dlls are the latest version before launching game2.exe for testing
- Check the timestamp of game2.ese and dlls to ensure they match the latest build time
