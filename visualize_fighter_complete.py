#!/usr/bin/env python3
"""
Parse and visualize all patches from demo_fighter_10 actor mesh
"""
...
def parse_msh_file_complete(filename):
    ...
    patch_blocks = re.findall(r'PATCH \d+.*?PATCH_END', content, re.DOTALL)
    
    print(f"Found {len(patch_blocks)} patch blocks in file")
    ...
    for patch_idx, block in enumerate(patch_blocks):
        ...
        if vertices:
            ...
            print(f"  Patch {patch_idx}: Grid {grid_w}x{grid_h}, {len(vertices)} vertices, Bitmap {bitmap_id}")
...
def visualize_all_patches(patches, title="All Patches", view_angles=(20, 45)):
    ...
    all_faces = []
    colors = ['cyan', 'magenta', 'yellow', 'red', 'green', 'blue', 'orange', 'purple', 'brown', 'pink', 'lightgreen', 'lightblue']
...
def main():
    ...
    views = [
        ('demo_fighter_10_complete_isometric.png', (20, 45), 'Isometric'),
        ('demo_fighter_10_complete_front.png', (0, 0), 'Front'),
        ('demo_fighter_10_complete_top.png', (90, 0), 'Top'),
        ('demo_fighter_10_complete_side.png', (0, 90), 'Side'),
        ('demo_fighter_10_complete_angle1.png', (25, -45), 'Angle 1'),
        ('demo_fighter_10_complete_angle2.png', (25, 135), 'Angle 2'),
    ]
    
    for filename, (elev, azim), label in views:
        print(f"Creating {label} view...")
        fig = visualize_all_patches(all_patches, f"Demo Fighter 10 - {label} ({len(all_patches)} patches)")
